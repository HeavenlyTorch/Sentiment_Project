...
def query_for_usage():
    ...
    url = 'https://api.apify.com/v2/acts?token=apify_api_OxEat2Vcy6DI6ww9VXhsw9AZoZbO8f1izVuj' + st.secrets["apify_api_OxEat2Vcy6DI6ww9VXhsw9AZoZbO8f1izVuj"]
    r = requests.get(url)
    d = json.loads(r.text)
...
def create_wordcloud(desc_dataset: list):
    ...
    figs = []
    for title, words in [('Positive words', pos_words), ('Negative words', neg_words),
