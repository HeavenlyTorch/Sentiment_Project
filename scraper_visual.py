...
def show_scraper():
    ...
    if submit:
        ...
        if len(rating_dataset) > 0:
            ...
            fig = scraper.show_ratings(rating_dataset, rating_round=rating_round_d[st.session_state.rating_round],
                                       plot_type=st.session_state.plot_type)
            st.pyplot(fig)
            multiplier = 1 if st.session_state.rating_round == 'Continuous (0 to 1)' else rating_round_d[
                st.session_state.rating_round]
            st.write(
                f'<span class=big-font><b>Average <u>raw</u> rating:</b> `{np.mean(rating_dataset) * multiplier: .2f} / {multiplier}`</span>',
                unsafe_allow_html=True)
        ...
        if len(desc_dataset) > 0:
            ...
            col1, col2 = st.columns(2)
            with col1:
                fig, mean = scraper.eval_sentiment(desc_dataset, amazon_model, utils.AMAZON_ROW_LABELS,
                                                   utils.AMAZON_ROW_VALUES, 'Rating from 1 - 5 stars')
                st.pyplot(fig)
                st.write(f'<span class=big-font><b>Average rating from <u>1 - 5</u>:</b> `{mean: .2f} / 5`</span>',
                         unsafe_allow_html=True)
            with col2:
                fig, mean = scraper.eval_sentiment(desc_dataset, twitter_model, utils.TWITTER_ROW_LABELS,
                                                   utils.TWITTER_ROW_VALUES, 'Negative / Neutral / Positive')
                st.pyplot(fig)
                st.write(f'<span class=big-font><b>Average rating from <u>0 / 0.5 / 1</u>:</b> `{mean: .2f} / 1`</span>',
                         unsafe_allow_html=True)
